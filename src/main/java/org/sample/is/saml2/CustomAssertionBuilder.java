...
/**
 * Custom SAML2 Assertion builder
 */
public class CustomAssertionBuilder implements SAMLAssertionBuilder {
    ...
    public Assertion buildAssertion(SAMLSSOAuthnReqDTO authReqDTO, DateTime notOnOrAfter, String sessionId) throws IdentityException {
        try {
            ...
            log.info(authReqDTO.getUser().getTenantDomain().toString() + " ------authReqDTO.getUser().getTenantDomain()------");

            log.info(
                    PrivilegedCarbonContext.getThreadLocalCarbonContext().getTenantDomain().toString()
